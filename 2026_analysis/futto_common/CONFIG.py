...
TASK_CONFIGS = {
    'task01': {
        ...
        # ★ トルク帰属定義 (Task01)
        # マーカーが複数の関節の中継点になる場合(例:ふくらはぎ)に対応するため、
        # ゴム(Line)単位でどの関節のトルクとして働くかを定義します。
        'TORQUE_LINE_JOINTS': {
            # Hip 関節をまたぐゴム
            'Front_Upper_In': 'Hip',
            'Front_Upper_Out': 'Hip',
            ...
            'Back_Thigh_Out': 'Knee',
            'Back_Thigh_In': 'Knee',
            'Front_Knee_Upper_Out': 'Knee',
            'Front_Knee_Upper_In': 'Knee',
            'Front_Knee_Lower_Out': 'Knee',
            'Front_Knee_Lower_In': 'Knee',
            'Back_Knee_Out': 'Knee',
            'Back_Knee_In': 'Knee',
        ...
        # ★ 仮想関節定義 (Task01)
        'JOINT_CENTER_DEFS': {
            'Hip':   {'type': 'ratio_1_3_between_mids_offset_z', 'markers': [16012, 16014, 16000, 15960], 'offset_z': 0.0},
    ...
    'task02': {
        ...
        'NATURAL_LENGTHS': {
            "Front_Upper_In": 150.0,       "Front_Upper_Out": 200.0,
        ...
        # ★ 仮想関節定義 (Task02)
        'JOINT_CENTER_DEFS': {
            # Hip: 57960 と 57958 の中点 M から X軸(進行)方向に -50mm, Z軸(鉛直)方向に -10mm ずらした点
            #   offset_x < 0 = 後方にずらす (X=進行正のため)
            'Hip':   {'type': 'midpoint_offset_xz',
                      'markers': [57960, 57958],
                      'offset_x': -100.0,
                      'offset_z': -50.0},
